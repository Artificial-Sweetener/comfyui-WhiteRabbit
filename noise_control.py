...
def _sobel_mag(y):    # y: NHWC 1ch
    kx = torch.tensor([[-1,0,1],[-2,0,2],[-1,0,1]], dtype=torch.float32).view(1,1,3,3).to(y.device)
    ky = torch.tensor([[-1,-2,-1],[0,0,0],[1,2,1]], dtype=torch.float32).view(1,1,3,3).to(y.device)
...
def _gauss1d(sigma, r):
    if r <= 0: return torch.tensor([1.0], dtype=torch.float32)
...
def _upsample_mask(mask_tile, H, W, mode="nearest"):
    ...
    t = F.interpolate(t, size=(H,W), mode=("bilinear" if mode=="bilinear" else "nearest"),
...
class PixelHold:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "ref_index": ("INT", {"default": 0, "min": 0, "max": 999999,
                ...
                "tau_luma": ("FLOAT", {"default": 1.5/255.0, "min": 0.0, "max": 4.0/255.0, "step": 0.0005,
                    "tooltip": "Manual brightness threshold when Auto is OFF. Lower = stricter (more locking)."}),
                "tau_grad": ("FLOAT", {"default": 0.02, "min": 0.0, "max": 1.0, "step": 0.001,
                    "tooltip": "How much edge change to allow. Lower protects edges more."}),
                "mode": (["tile", "pixel"], {"default": "tile",
                ...
                "tau_edge_low": ("FLOAT", {"default": 1.5/255.0, "min": 0.0, "max": 0.25, "step": 0.0005,
    ...
    def apply_hold(
        ...
        for i in range(B):
            ...
            if mode == "tile":
                sY = _mad_tiles(dY, tile_size) if score_mode == "mad_tile" else _avgpool_tiles(dY, tile_size)
                sG = _mad_tiles(dG, tile_size) if score_mode == "mad_tile" else _avgpool_tiles(dG, tile_size)
                mask = (sY < tau_luma_eff).to(torch.float32) * (sG < float(tau_grad)).to(torch.float32)
                mask = _upsample_mask(mask, H, W, mode="nearest")
            else:
                mask = (dY < tau_luma_eff).to(torch.float32) * (dG < float(tau_grad)).to(torch.float32)
            ...
            if edge_band:
                ...
                if feather_sigma > 0:
                    band = _blur_nhwc(band.to("cpu"), float(feather_sigma)).to(dev).clamp_(0.0,1.0)
            ...
            if apply == "all":
                ...
                HF_f   = f_cpu - LF_f
                LF_mix = mask.to("cpu") * LF_r[i:i+1] + (1.0 - mask.to("cpu")) * LF_f
...
class BlackSpotCleaner:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "grad_guard": ("BOOLEAN", {"default": True,
            ...
            "optional": {
                "reference": ("IMAGE", {"tooltip": "Optional external reference floor (1×H×W×C). Resized if needed."}),
                "ref_source": (["none","external","batch_index"], {"default": "none",
                ...
                "tau_down": ("FLOAT", {"default": 2.0/255.0, "min": 0.0, "max": 0.5, "step": 0.0005,
    ...
    def clean(
        ...
        if dev.type == "cuda": out_lin = out_lin.to("cpu")
