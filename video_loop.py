...
class RollFrames:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "images": ("IMAGE", {
                    "tooltip": "Your clip (frames×H×W×C)."
                }),
                "offset": ("INT", {
...
class AutocropToLoop:
    ...
    RETURN_NAMES = ("cropped_clip", "end_crop_frames", "cropped_length", "score", "diagnostics_csv")
    ...
    def _precompute_adjacent_metrics(self, clip_nhwc_dev, kind, use_ssim, ds_scales, use_exp, use_flow):
        ...
        import numpy as np
        import torch
        B = int(clip_nhwc_dev.shape[0]); N = max(0, B - 1)
        result = {}
        if N == 0:
    ...
    def _flow_mag_mean(self, a_nhwc, b_nhwc, max_side=256):
        ...
        if b.ndim == 4 and b.shape[0] == 1: b = b[0]
        ...
        def to_gray(x: np.ndarray) -> np.ndarray:
            ...
            if x.ndim == 3:
                c = x.shape[-1]
                if c == 1: return x[..., 0]
                if c == 3: return cv2.cvtColor(x, cv2.COLOR_RGB2GRAY)
                if c == 4: return cv2.cvtColor(x, cv2.COLOR_RGBA2GRAY)
                return x.mean(axis=-1).astype(x.dtype)
            x2 = np.squeeze(x)
            if x2.ndim == 2: return x2
            if x2.ndim == 3: return x2.mean(axis=-1).astype(x2.dtype)
            return None
        ...
        try:
            flow = cv2.calcOpticalFlowFarneback(a_g, b_g, None, 0.5, 3, 21, 3, 5, 1.1, 0)
    ...
    def find_and_crop(self,
        ...
        with torch.no_grad():
            with amp_ctx():
                ...
                pre, x_nchw_dev = self._precompute_adjacent_metrics(
                    ...
                    use_exp=use_exposure_guard, use_flow=use_flow_guard
                )
                D_adj, S_adj, E_adj, F_adj = pre["D_adj"], pre["S_adj"], pre["E_adj"], pre["F_adj"]

                D_seam_tab, S_seam_tab, E_seam_tab = self._precompute_seam_tables(
                    x_nchw_dev=x_nchw_dev, W=W, kind=kind,
                    use_ssim=use_ssim_similarity, ds_scales=ds_scales
                )
        ...
        for extra in range(0, total_candidates):
            ...
            if include_global_median_step and keep >= 3: chosen_D.append(D_adj[:keep-1].median())
            if not chosen_D and keep >= 2: chosen_D = [D_adj[0]]
            D_target = float((chosen_D[0] if len(chosen_D) == 1 else torch.stack(chosen_D).median()).item())

            if use_ssim_similarity and S_adj.numel() > 0 and keep >= 2:
                chosen_S = []
                if include_first_step: chosen_S.append(S_adj[0])
                if include_last_step:  chosen_S.append(S_adj[last_idx - 1])
                if include_global_median_step and keep >= 3: chosen_S.append(S_adj[:keep-1].median())
                S_target = float((chosen_S[0] if (chosen_S and len(chosen_S) == 1)
                                  else (torch.stack(chosen_S).median() if chosen_S else torch.tensor(0.0, device=S_adj.device))).item())
            else:
                S_target = 0.0
            ...
            if use_exposure_guard and keep >= 2:
                ...
                if include_global_median_step and keep >= 3:
                    ...
                    E_target = float(torch.stack([e_first, e_last, e_med]).median().item())
                else:
            ...
            cost_exp  = abs(E_seam - E_target) / (E_target + eps) if (use_exposure_guard and E_target > 0.0) else 0.0
            cost_flow = abs(F_seam - F_target) / (F_target + eps) if (use_flow_guard and F_target > 0.0) else 0.0

            score = (weight_step_size * cost_step
                     + weight_similarity * cost_sim
                     + weight_exposure  * cost_exp
                     + weight_flow      * cost_flow)

            rows.append(f"{extra},{score:.6f},{D_seam:.6f},{D_target:.6f},{S_seam:.6f},{S_target:.6f},{E_seam:.6f},{E_target:.6f},{F_seam:.6f},{F_target:.6f}")
        ...
        return (cropped, int(best_extra), int(final_keep), float(best_score), diagnostics_csv)
