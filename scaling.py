...
"""
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                ...
                "tile_size": ("INT", {
                    "default": 0, "min": 0, "max": 2048, "step": 32,
    ...
    def upscale(
        ...
        for s, e in spans(B, int(max_batch_size)):
            ...
            while oom:
                try:
                    steps = sub.shape[0] * comfy_utils.get_tiled_scale_steps(
                        sub.shape[3], sub.shape[2], tile_x=tile, tile_y=tile, overlap=overlap
                    )
            ...
            out_chunks.append(torch.clamp(sr.movedim(-3, -1), 0.0, 1.0).to("cpu", non_blocking=True))
...
def _fit_keep_ar_divisible(sw: int, sh: int, tw: int, th: int, d: int) -> Tuple[int, int]:
    if d <= 1:
...
def _scale_then_crop_divisible(sw: int, sh: int, req_w: int, req_h: int, d: int) -> Tuple[int, int, int, int]:
    """
    ...
    if src_long_is_h:
        if req_h_div == 0:
            raise ValueError(f"AR Scale + Divisible Crop: requested height {req_h}px < divisible_by {d}.")
        scale = req_h_div / sh
        rh = req_h_div
        rw = max(1, int(round(sw * scale)))
        if rw < d:
            raise ValueError(f"AR Scale + Divisible Crop: scaled width {rw}px < divisible_by {d}.")
        if req_w_div == 0:
            raise ValueError(f"AR Scale + Divisible Crop: requested width {req_w}px < divisible_by {d}.")
        ...
        if rh < d:
            raise ValueError(f"AR Scale + Divisible Crop: scaled height {rh}px < divisible_by {d}.")
        if req_h_div == 0:
            raise ValueError(f"AR Scale + Divisible Crop: requested height {req_h}px < divisible_by {d}.")
...
def _crop_offsets(pos: str, in_w: int, in_h: int, out_w: int, out_h: int) -> Tuple[int, int]:
    dx = max(0, in_w - out_w)
    dy = max(0, in_h - out_h)
    mapx = {"top-left": "left", "left": "left", "bottom-left": "left",
            "top": "center", "center": "center", "bottom": "center",
            "top-right": "right", "right": "right", "bottom-right": "right"}
    mapy = {"top-left": "top", "top": "top", "top-right": "top",
            "left": "center", "center": "center", "right": "center",
            "bottom-left": "bottom", "bottom": "bottom", "bottom-right": "bottom"}
    lx = {"left": 0, "center": dx // 2, "right": dx}.get(mapx.get(pos, "center"), dx // 2)
    ly = {"top": 0, "center": dy // 2, "bottom": dy}.get(mapy.get(pos, "center"), dy // 2)
    return lx, ly


def _parse_pad_color(s: str, c: int, device: torch.device, dtype: torch.dtype) -> torch.Tensor:
    s = (s or "").strip()
    if not s:
    ...
    v = torch.tensor([rgb[0] / 255.0, rgb[1] / 255.0, rgb[2] / 255.0], device=device, dtype=dtype)
    if c == 1:
        return v[:1]
    if c == 4:
...
class BatchResizeWithLanczos:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "image": ("IMAGE", {"tooltip": "Input batch (B,H,W,C) in [0,1] float.\nProcessed on GPU."}),
                "width": ("INT", {
                ...
                "height": ("INT", {
                    "default": 576, "min": 1, "max": 16384, "step": 1,
    ...
    def process(
        ...
        if mode == "stretch":
            ...
            if width <= 0 or height <= 0:
                raise ValueError("AR Scale + Divisible Crop requires non-zero width and height.")
            rw, rh, out_w, out_h = _scale_then_crop_divisible(W, H, int(width), int(height), d)
        ...
        for s, e in _chunk_spans(B, int(max_batch_size)):
            ...
            y = lanczos_resize(x, height=resize_to[0], width=resize_to[1],
                               a=int(sinc_window), precision=str(precision), clamp=True, chunk_size=0)
            ...
            if crop_like:
                ...
                color = _parse_pad_color(pad_color, C, y.device, y.dtype).view(1, C, 1, 1)
            ...
            if isinstance(mask, torch.Tensor):
                ...
                if crop_like:
                    ...
                    base = torch.zeros((m_res.shape[0], 1, out_h, out_w), device=m_res.device, dtype=m_res.dtype)
